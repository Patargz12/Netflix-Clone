...
import { adaptTVArrayToMovies, adaptTVToMovie } from '@/utils/tvAdapter';
...
export const TVPage = () => {
  ...
  const handleMovieSelect = (movie: TV) => {
    // FIXED: We now update both the selectedShow (for the featured section) and dialogShow (for the dialog)
    // This ensures we're tracking both states separately
    setSelectedShow(movie);
    setDialogShow(movie);
    setShowDialog(true);

    // ADDED: Debug logging to track which show is being selected
    console.log(`Selected show: ${movie.name} (ID: ${movie.id})`);
  };

  const handleFeaturedPlayClick = () => {
    if (selectedShow) {
      // FIXED: When clicking play on the featured show, we use handleMovieSelect to ensure
      // both selectedShow and dialogShow are updated consistently
      ...
      <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
        {selectedShow && (
          <>
            ...
            />
            {/* FIXED: Critical change - we now pass dialogShow to the MovieDialog instead of selectedShow */}
            {/* This ensures the dialog always shows the correct selected show */}
            {dialogShow && (
              <MovieDialog
          ...
          {searchQuery ? (
            <div className="">
              <MovieGrid
                ...
                onPlayClick={() => {
                  // FIXED: Only open the dialog if we have a dialogShow set
                  if (dialogShow) {
                    setShowDialog(true);
                  }
                }}
